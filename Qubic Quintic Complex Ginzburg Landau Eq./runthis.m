...
% Author: Gholam-Ali Zakeri Emmanuel Yomba
% Case 2A.
function runthis()
% Figure 6
...
for i = 1:length(X)
   for j = 1:length(t) 
        if t(1, i) == 0 || X(1, i) == 0
            t_x = X(1, i);
            t_t = t(1, i);            
            amplitude(i, j) =  A(t_x, t_t, t_n, t_r, t_w, t_k1, t_omega1, alpha1, b) + ...
                A(t_x, t_t, t_n, t_r, t_w, t_k1, t_omega1, alpha1, b) * ((0.1 *rand(1)));
         
            amplitude_b(i, j) =  B(t_x, t_t, t_mu, t_r, t_w, t_k2, t_omega2, alpha2, b) + ...

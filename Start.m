...
t = 0:1/8:5;
dx = 1;
[X,Y] = meshgrid(-20:dx:20, -0:1/40:1);
t = meshgrid(t);
...
%          end
           if t(1, k) == 0 ||  X(1, k) == 0
               t_x = X(1, k);
               t_t = t(1, k); 
               amplitude(k, j) =  A(t_x, t_t, t_n, t_r, t_w, t_k1, t_omega1, alpha1, b) + ...
                 A(t_x, t_t, t_n, t_r, t_w, t_k1, t_omega1, alpha1, b) * ((0.1 *rand(1))); 
                A_(k, j) = amplitude(k, j);
           end
    end
end

% Populate the amplitude matrix
integrand = @(x) x*1;
...
for k = 2:length(X)-1
    for j = 2: length(t)-1
        ...
        t_t = t(1, k);
        amplitude(k, j) =  A(t_x, t_t, t_n, t_r, t_w, t_k1, t_omega1, alpha1, b);
        amplitude_b(k, j) =  B(t_x, t_t, t_mu, t_r, t_w, t_k2, t_omega2, alpha2, b);
        % k = x
        % compute split step crank
        % nicholson linear part
        A_linear  = gamma1 * (1/2) * (amplitude(k+1,j) - 2 * amplitude(k, j) + amplitude(k-1, j) + ...
            amplitude(k+1, j-1) - 2 * amplitude(k, j-1) + amplitude(k-1, j-1))/(2*(dx^2)) - ...
            t_v *(1/2) * ((( amplitude(k+1, j) -  amplitude(k-1, j))/2*dx) + (( amplitude(k+1, j-1) -  amplitude(k-1, j-1))/2*dx));
        A_next = chi1 * amplitude(k, j) - beta1(xi1, xi2, beta2) * (abs(amplitude(k, j))^2) * amplitude(k, j) - ...
            delta1 * (abs(amplitude(k, j))^4) * amplitude(k, j) - xi1 * (abs(amplitude_b(k, j))^2) * amplitude(k, j);
        A_previous = chi1 * amplitude(k, j-1) - beta1(xi1, xi2, beta2) * (abs(amplitude(k, j-1))^2) * amplitude(k, j-1) - ...
            delta1 * (abs(amplitude(k, j-1))^4) * amplitude(k, j-1) - xi1 * (abs(amplitude_b(k, j-1))^2) * amplitude(k, j-1);
        A_nonlinear = (1/2) * (A_next + A_previous);
        A_t(k, j) = A_linear + A_nonlinear;
        A_(k, j) = A_t(k, j) ; %integral(@(x)integrand(x),t(k, j-1),t(k, j));
    end
end
...
%             amplitude_b(z, j) =  B(t_x, t_t, t_mu, t_r, t_w, t_k2, t_omega2, alpha2, b);
%             
%             amplitude = computeCoupledEquationOne(amplitude, amplitude_b, chi1, gamma1, beta1(xi1, xi2, beta2), delta1, xi1);
%         end
%    end
% end
% 
...
subplot(2,1,1)
colormap(cool);
s1 = surf(t,Y,abs(A_),'FaceAlpha',1);
% s1.EdgeColor = 'none'; 
